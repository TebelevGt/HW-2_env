...
## 1.4 Генерируем дата сет для обучения
...
Цель проекта — обучить компактную языковую модель (1.5B параметров) решать задачи многошаговой навигации (поиска пути в графе/среде).
...
Главная проблема при решении таких задач малыми моделями — высокая доля галлюцинаций, неспособность удерживать контекст длинного пути и склонность к «взлому награды» (reward hacking) при стандартном обучении с подкреплением (RLHF).
