# 1. Повторимость экспериментов
в папке notebooks забираем и через kaggle/colab запускаем следующие ноутбуки:
- Запускаем ноутбук 1 для генерации hard train и hard val дата сетов
- С помощью ноутбука 2 генерим золотые траектории
- С помощью ноутбука 3 обучаем весь зоопарк моделей
- Валидируем зоопарк моделей с помощью ноутбука 4
 

# 2. Оптимизация агентных способностей Qwen 2.5 (0.5B) с помощью SFT и GRPO
...
| :--- | :--- | :--- | :--- | :--- |
| **Шаг 1** | Baseline оценка | *Обучение не проводится* | `Hard-val` | Замеряем `pass@k` и длину генерации на базовой модели Qwen2.5-0.5B-Instruct без дообучения. Доказываем, что на Hard-val `pass@128=0`. |
| **Шаг 2** | GRPO-only (без gold) | `curriculum+hard_train.pkl` (стандартный, без золотых траекторий) | `Hard-val` | Обучаем базовую модель только с помощью Curriculum GRPO и reward-функции через верификатор. |
| **Шаг 3.1** | SFT-only (Опционально) | `gold_train_128.pkl` (только собранные gold trajectories) |  `Hard-val` | Обучаем базовую модель классическим Supervised Fine-Tuning на эталонных решениях. В задании сказано, что это полезно для декомпозиции эффектов. |
| **Шаг 3.2** | SFT $\rightarrow$ GRPO | `curriculum+hard_train.pkl` (с reward-функцией) | `Hard-val` | Берем веса модели, получившиеся на шаге 3.1 (после SFT), и дообучаем их с помощью Curriculum GRPO. |
| **Шаг 4** | SRFT (Выбранный метод) | `curriculum+gold_train.pkl` (смесь on-policy генераций и gold trajectories) | `Hard-val` | Берем базовую модель (Baseline) и обучаем её гибридным методом, совмещая SFT-сигнал и RL-награды в одном цикле обучения. |
...
![alt text](results/results_dashboard.png)

![alt text](results/training_dynamics.png)